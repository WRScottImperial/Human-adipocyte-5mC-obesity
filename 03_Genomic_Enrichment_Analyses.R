...
### 1. Generation of 1000 permutations of of background CG sites, each equal in length to the number of sentinels, with similar mean (sd) methylation levels: 

# criteria i. difference in mean between sentinel and permutation < X; 
# criteria ii. difference in standard deviation (sd) between sentinel and permutation < Y; 
# criteria iii. >5-kb distance between sentinel and permutation (i.e. not at the same genomic locus). 

# For each sentinel, difference in mean (< X) and sd (< Y) is based on a sliding scale starting at mean < 0.025 (i.e. 2.5%) and sd < 0.0025 
...
msd = anno[,c(52:53)]
colnames(msd)=c("meanMeth","sdMeth")
